...
**核心修复**：`Proj_MHSA` 从 mmcv `MultiheadAttention`（包 atorch FA，forward 带 identity 残差）改造为 torch 原生 `nn.MultiheadAttention` 时丢失了残差连接，导致推理 mIoU 从基线环境基准 **0.5515** 坍缩至 **0.0089**；补回 `x = x + identity` 后恢复至 **0.5513**，与基准完全对齐。
...
3. 按 `docs/部署文档_v1.md` 配置环境（基础镜像 `pytorch/pytorch:2.1.2-cuda12.1-cudnn8-devel`，可按需自备私有镜像仓库，依赖见 `scripts/requirements_l20.txt`）
